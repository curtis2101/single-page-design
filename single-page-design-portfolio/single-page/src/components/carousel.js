            ...
            {ImageData.map((slide, index) => {
                return (
                    <div className={index === current ? 'opacity-100 duration-1000 transform flex justify-center items-center' : 'opacity-0 duration-1000 flex justify-center items-center'} key={index}>
                        {index === current && (<>
                        <img src={ImageData[current <= 1 ? length - 2 : current -1].image} alt='multiple images' className='mt-16 mr-6 h-44 w-42 rounded-xl' />
                        <img src={slide.image} alt='multiple images' className='mt-16 h-44 w-44 rounded-xl' />
                        <img src={ImageData[current >= length -1 ? 0:current +1].image} alt='multiple images' className='mt-16 ml-6 h-44 w-42 rounded-xl' />
                        </>
                        )}
                    </div>
    );
